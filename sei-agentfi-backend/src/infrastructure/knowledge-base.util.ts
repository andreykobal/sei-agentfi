...
# SEI AgentFi Platform Knowledge Base

## 1️⃣ Project Name
SEI: AgentFi - AI-native DeFi platform for real-time insights and autonomous trading
...
Blockchain (Uniswap) serves as the source of truth, from which we read events like token launches, swaps, and liquidity changes. These events are processed, projections are built into MongoDB (e.g., for token stats, prices, volumes, liquidity), and via separate CQRS routes, we deliver ready-made data for analytics, market dashboards, and insights.

## 7️⃣ Why It Matters / Benefits / Impact
The platform removes the main barrier in DeFi between a token idea and a functioning market with real activity. It reduces risks for investors through transparent growth mechanisms, removes barriers for projects (no need for expensive market makers), and creates a healthier token ecosystem where market activity is managed by AI agents, not manipulation.

## 🔥 One-liner Pitch
"AI-native DeFi platform delivering real-time market intelligence and agentic trading through autonomous AI agents — from automated token creation to AMM liquidity, shaping the future of DeFi."
`;

// Helper function to get specific sections of the knowledge base
export const getKnowledgeBaseSection = (section: string): string => {
  const sections: Record<string, string> = {
    ...
    solution:
      "We are creating a standardized, automated method for launching and supporting DeFi tokens with real liquidity and market activity, managed by AI agents that provide real-time insights for decision-making.",
    audience:
      "Token Developers, DeFi Traders, Meme Coin and Experimental Token Creators",
    tech: "SEI Network, Solidity, Uniswap V4, Ponder, Anthropic Claude, Next.js, Node.js, MongoDB",
    pitch:
      "AI-native DeFi platform delivering real-time market intelligence and agentic trading through autonomous AI agents — from automated token creation to AMM liquidity, shaping the future of DeFi.",
  };

  return sections[section.toLowerCase()] || PLATFORM_KNOWLEDGE_BASE;
};
